...
@Component({
  ...
  imports: [CommonModule, ReactiveFormsModule, MatButtonModule, MatCardModule, MatDatepickerModule, MatFormFieldModule, MatIconModule,RouterModule, MatInputModule,MatSelectModule],
...
export class ExpenseAddEditComponent {
  ...
  categories = ["Food","Bills","Housing","Travel","Leisure","Health","Transportation","Shopping","Education","Pets"]
  ...
  generateNewId(): number {
    ...
    const lastId = Math.max(...this.expenseService.expenses().map(expense => expense.id));
    return lastId + 1;
  }
